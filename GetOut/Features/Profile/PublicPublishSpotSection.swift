...
struct PublicPublishSpotSection: View {
    ...
    private var isPublished: Bool { !spot.publicRecordName.isEmpty }
        ...
        if FeatureFlags.publicSocialEnabled {
                    ...
                    Button { showPublishConfirmation = true } label: {
                        HStack {
                            if coordinator.isPublishingSpot {
                                ProgressView().tint(Theme.Colors.accentGreen)
                            } else {
            ...
            .alert("Share this exact location publicly?", isPresented: $showPublishConfirmation) {
                Button("Share publicly") {
                    Task { await coordinator.publishSpot(spot, owner: owner, in: modelContext) }
                }
