...
final class PublicSocialCoordinator {
    ...
    let service: PublicSocialService

    private init(service: PublicSocialService? = nil) {
        if let service {
            self.service = service
        } else {
            self.service = PublicSocialServiceFactory.makeLive()
        }
    }
    ...
    func bootstrapIdentity(for profile: Profile, in context: ModelContext) async {
        ...
        profile.supabaseUserID = userRecordName
        try? context.save()
    ...
    func restoreCurrentProfile(in context: ModelContext, session: SessionStore) async {
        ...
        do {
            ...
            let identityChanged = session.currentSupabaseUserID != userRecordName
            if identityChanged {
    ...
    func createProfile(
        ...
        do {
            ...
            PublicSocialCacheStore.apply(dto, to: profile)
            if isNewProfile {
    ...
    func publishSpot(_ spot: Spot, owner: Profile, in context: ModelContext) async -> Bool {
        ...
        owner.supabaseUserID = currentUserRecordName
        ...
        do {
            ...
            PublicSocialCacheStore.apply(dto, to: spot)
    ...
    func block(userRecordName: String, in context: ModelContext) {
        ...
        if let blockedID = UUID(uuidString: userRecordName),
           let blockerID = currentUserRecordName.flatMap(UUID.init(uuidString:)) {
            Task { try? await SupabasePrivateDataService.shared.setBlock(blockedUserID: blockedID, blockerID: blockerID, isBlocked: true) }
        }
    ...
    func unblock(userRecordName: String, in context: ModelContext) {
        ...
        if let blockedID = UUID(uuidString: userRecordName),
           let blockerID = currentUserRecordName.flatMap(UUID.init(uuidString:)) {
            Task { try? await SupabasePrivateDataService.shared.setBlock(blockedUserID: blockedID, blockerID: blockerID, isBlocked: false) }
        }
    ...
    func deleteCurrentAccount(in context: ModelContext, session: SessionStore) async -> Bool {
        ...
        do {
            ...
            try clearLocalAccountData(in: context)
    ...
    func signOut(in context: ModelContext, session: SessionStore) async -> Bool {
        accountError = nil
        do {
    ...
    private func clearLocalAccountData(in context: ModelContext) throws {
        ...
        for item in (try? context.fetch(FetchDescriptor<Rating>())) ?? [] { context.delete(item) }
