...
function AddForm() {
  const {publishSpot, circles} = useApp(); const access = useCannabisAccess(); const [title, setTitle] = useState(''); const [details, setDetails] = useState(''); const [category, setCategory] = useState<Category>('views'); const [tag, setTag] = useState(''); const [tags, setTags] = useState<string[]>([]); const [photos, setPhotos] = useState<string[]>([]); const [query, setQuery] = useState(''); const [coordinate, setCoordinate] = useState({latitude: 43.6532, longitude: -79.3832}); const [address, setAddress] = useState<any>({}); const [cannabis, setCannabis] = useState(false); const [busy, setBusy] = useState(false); const [isPublic, setIsPublic] = useState(true); const [circleIds, setCircleIds] = useState<string[]>([]);
