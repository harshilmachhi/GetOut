...
import {normalizeTag, similarTags} from '@/lib/tags';
...
function AddForm() {
  const {publishSpot, circles, spots} = useApp(); const access = useCannabisAccess(); const [title, setTitle] = useState(''); const [details, setDetails] = useState(''); const [category, setCategory] = useState<Category>('views'); const [tag, setTag] = useState(''); const [tags, setTags] = useState<string[]>([]); const [photos, setPhotos] = useState<string[]>([]); const [query, setQuery] = useState(''); const [coordinate, setCoordinate] = useState({latitude: 43.6532, longitude: -79.3832}); const [address, setAddress] = useState<any>({}); const [cannabis, setCannabis] = useState(false); const [busy, setBusy] = useState(false); const [isPublic, setIsPublic] = useState(true); const [circleIds, setCircleIds] = useState<string[]>([]);
  ...
  const knownTags = useMemo(()=>spots.flatMap(spot=>spot.tags),[spots]);
  const tagSuggestions = useMemo(()=>similarTags(tag,knownTags,tags),[tag,knownTags,tags]);
  const addTag = (candidate=tag) => { const value=normalizeTag(candidate); if(value&&!tags.includes(value))setTags(current=>[...current,value]);setTag(''); };
  const publish = async () => { const isCannabis = cannabis || tags.some(value => ['weed-friendly','weed','cannabis','420'].includes(value)); if (!title.trim() || !coordinate.latitude || !coordinate.longitude) return Alert.alert('Complete the spot', 'Add a name and pinned location.'); if (!isPublic && !circleIds.length) return Alert.alert('Choose who can see it', 'Share publicly, with at least one Circle, or both.'); if (isCannabis && !access.ageConfirmed) return Alert.alert('Legal age confirmation','Cannabis-related spots are limited to adults physically in Canada or California and to spots located there.',[{text:'Cancel'},{text:'I am of legal age',onPress:()=>access.setAgeConfirmed(true)}]); if (isCannabis && !access.jurisdiction) return Alert.alert('Current location required','Allow location access so GetOut can confirm your current jurisdiction.'); if (isCannabis && (!isCannabisJurisdiction(access.jurisdiction?.countryCode ?? '', access.jurisdiction?.administrativeArea ?? '') || !isCannabisJurisdiction(address.isoCountryCode ?? '', address.region ?? ''))) return Alert.alert('Location not eligible', 'Weed-friendly spots require both you and the pinned spot to be in Canada or California.'); try { setBusy(true); const spot = await publishSpot({title: title.trim(), details: details.trim(), latitude: coordinate.latitude, longitude: coordinate.longitude, address: [address.name, address.street, address.city].filter(Boolean).join(', '), city: address.city ?? '', neighborhood: address.district ?? address.subregion ?? '', category, rating: 0, visit_hour: new Date().getHours(), visit_weekday: new Date().getDay() + 1, tags, contains_cannabis: isCannabis, country_code: address.isoCountryCode ?? '', administrative_area: address.region ?? '', photoUris: photos, isPublic, circleIds}); Alert.alert('Spot shared!', undefined, [{text: 'View spot', onPress: () => router.push(`/spot/${spot.id}`)}]); } catch (e) { Alert.alert('Could not share', e instanceof Error ? e.message : String(e)); } finally { setBusy(false); } };
  return <Screen><Title>Add a Spot</Title><SectionTitle>1  Choose a location</SectionTitle><View style={styles.locationSearch}><View style={styles.searchRow}><Field style={{flex: 1}} value={query} onChangeText={value => {setQuery(value); setSearchFocused(true)}} onFocus={() => setSearchFocused(true)} placeholder="Search for a place or address" returnKeyType="search" onSubmitEditing={search}/><Pressable onPress={search} style={styles.smallButton}>{searching ? <ActivityIndicator color={colors.green}/> : <Ionicons name="search" size={20} color={colors.text}/>}</Pressable></View>{searchFocused && (searching || suggestions.length > 0) && <View style={styles.suggestions}>{searching && !suggestions.length && <View style={styles.searchingRow}><ActivityIndicator size="small" color={colors.green}/><Muted>Finding places…</Muted></View>}{suggestions.map((suggestion, index) => <Pressable key={suggestion.id} onPress={() => selectSuggestion(suggestion)} style={[styles.suggestion, index < suggestions.length - 1 && styles.suggestionDivider]}><View style={styles.pin}><Ionicons name="location" size={18} color={colors.green}/></View><View style={{flex: 1}}><Text numberOfLines={1} style={styles.suggestionTitle}>{suggestion.title}</Text>{!!suggestion.subtitle && <Text numberOfLines={2} style={styles.suggestionSubtitle}>{suggestion.subtitle}</Text>}</View><Ionicons name="chevron-forward" size={17} color={colors.subtle}/></Pressable>)}</View>}</View><PrimaryButton title="Use my current location" icon="locate" onPress={useMyLocation}/><MapView style={styles.map} region={{...coordinate, latitudeDelta: .025, longitudeDelta: .025}} onPress={(e: MapPressEvent) => reverse(e.nativeEvent.coordinate)}><Marker draggable coordinate={coordinate} onDragEnd={e => reverse(e.nativeEvent.coordinate)}/></MapView><Muted>{[address.name, address.street, address.city, address.region].filter(Boolean).join(', ') || 'Tap the map to pin an exact location'}</Muted>
    ...
    <View style={styles.searchRow}><Field style={{flex: 1}} value={tag} onChangeText={setTag} placeholder="Add a tag, like quiet or date-night" onSubmitEditing={()=>addTag()}/><Pressable onPress={()=>addTag()} style={styles.smallButton}><Text style={styles.add}>Add</Text></Pressable></View>{!!tagSuggestions.length&&<View style={styles.tagSuggestions}><Muted>Use an existing tag:</Muted><View style={styles.wrap}>{tagSuggestions.map(value=><Chip key={value} label={`#${value}`} onPress={()=>addTag(value)}/>)}</View></View>}<View style={styles.wrap}>{tags.map(v => <Chip key={v} label={`#${v}  ×`} onPress={() => setTags(x => x.filter(t => t !== v))}/>)}</View>
    ...
    <View style={styles.toggle}><View style={{flex: 1}}><Text style={styles.label}>Weed-friendly</Text><Muted>Shown only where legally eligible.</Muted></View><Switch value={cannabis} onValueChange={setCannabis} trackColor={{true: colors.green}}/></View>
    <SectionTitle>4  Choose who can see it</SectionTitle><View style={styles.audienceCard}><Pressable onPress={()=>setIsPublic(value=>!value)} style={styles.audienceRow}><View style={[styles.audienceIcon,isPublic&&styles.audienceIconActive]}><Ionicons name="earth" size={20} color={isPublic?colors.text:colors.muted}/></View><View style={{flex:1}}><Text style={styles.label}>Public</Text><Muted>Everyone can see the exact location.</Muted></View><Switch value={isPublic} onValueChange={setIsPublic} trackColor={{true:colors.orange}}/></Pressable>{circles.map(circle=>{const selected=circleIds.includes(circle.id);return <Pressable key={circle.id} onPress={()=>setCircleIds(values=>selected?values.filter(id=>id!==circle.id):[...values,circle.id])} style={styles.audienceRow}><View style={[styles.audienceIcon,selected&&styles.audienceIconActive]}><Ionicons name="people" size={20} color={selected?colors.text:colors.muted}/></View><View style={{flex:1}}><Text style={styles.label}>{circle.name}</Text><Muted>Only current members.</Muted></View><Ionicons name={selected?'checkmark-circle':'ellipse-outline'} size={25} color={selected?colors.green:colors.subtle}/></Pressable>})}{!circles.length&&<Pressable onPress={()=>router.push('/circles' as never)} style={styles.makeCircle}><Ionicons name="add-circle-outline" size={20} color={colors.green}/><Text style={styles.add}>Create a private Circle</Text></Pressable>}</View><Muted>{isPublic&&circleIds.length?'This spot will appear publicly and inside selected Circles.':isPublic?'This spot will be visible to everyone.':'Private spot: photos and coordinates require Circle membership.'}</Muted><PrimaryButton title={busy ? 'Sharing…' : 'Share spot'} disabled={busy} onPress={publish}/>
  </Screen>;
}
const styles = StyleSheet.create({locationSearch: {gap: spacing.sm}, searchRow: {flexDirection: 'row', gap: spacing.sm}, smallButton: {minWidth: 50, height: 50, borderRadius: radius.control, backgroundColor: colors.surface, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, alignItems: 'center', justifyContent: 'center'}, suggestions: {backgroundColor: colors.surface, borderRadius: radius.control, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.border, overflow: 'hidden'}, searchingRow: {minHeight: 56, paddingHorizontal: spacing.md, flexDirection: 'row', alignItems: 'center', gap: spacing.sm}, suggestion: {minHeight: 66, paddingHorizontal: spacing.md, paddingVertical: 11, flexDirection: 'row', alignItems: 'center', gap: 12}, suggestionDivider: {borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.faint}, pin: {width: 34, height: 34, borderRadius: 17, backgroundColor: 'rgba(107,153,97,.16)', alignItems: 'center', justifyContent: 'center'}, suggestionTitle: {color: colors.text, fontSize: 15, fontWeight: '700'}, suggestionSubtitle: {color: colors.muted, fontSize: 12, lineHeight: 17, marginTop: 2}, map: {height: 260, borderRadius: 20}, wrap: {flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm}, tagSuggestions:{gap:spacing.sm,padding:spacing.sm,backgroundColor:colors.surface,borderRadius:radius.control},photos: {flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm}, photo: {width: 94, height: 94, borderRadius: 14}, photoAdd: {width: 94, height: 94, borderRadius: 14, borderStyle: 'dashed', borderColor: colors.green, borderWidth: 1, alignItems: 'center', justifyContent: 'center'}, add: {color: colors.green, fontWeight: '700'}, toggle: {flexDirection: 'row', alignItems: 'center', padding: spacing.md, backgroundColor: colors.surface, borderRadius: 14}, label: {fontSize: 16, color: colors.text, fontWeight: '700'},audienceCard:{backgroundColor:colors.surface,borderRadius:radius.card,borderWidth:StyleSheet.hairlineWidth,borderColor:colors.border,overflow:'hidden'},audienceRow:{minHeight:72,padding:spacing.md,flexDirection:'row',alignItems:'center',gap:spacing.md,borderBottomWidth:StyleSheet.hairlineWidth,borderBottomColor:colors.faint},audienceIcon:{width:38,height:38,borderRadius:19,backgroundColor:colors.surface2,alignItems:'center',justifyContent:'center'},audienceIconActive:{backgroundColor:'rgba(107,153,97,.28)'},makeCircle:{padding:spacing.md,flexDirection:'row',alignItems:'center',justifyContent:'center',gap:spacing.sm}});
